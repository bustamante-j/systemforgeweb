...
**Previews** (`TemplatePreview`) are stills, not live sites. Every template with a `demoUrl` has one 1280×800 WebP in [src/assets/previews/](src/assets/previews/) named `<template.id>.webp`, and the component reaches that folder through a single eager `import.meta.glob` — so the build content-hashes each shot into `/assets`, which `vercel.json` caches `immutable` for a year, and the hash is what makes re-shooting a demo under the same filename safe. [scripts/shoot-previews.mjs](scripts/shoot-previews.mjs) takes the pictures; it needs playwright, which is deliberately not a dependency of the site (the script's header says how to run it). A template with no file on disk is not a broken card: the frame keeps its label, and the demo itself is still one click away through the card's "Live demo" link and the detail page's "Open the full demo".

Easy-to-break details:

- Shots are taken at a real 1280×800 desktop viewport and scaled down by the frame, because the demos use `100vh` heroes — a picture taken at card width would be a different layout, not a smaller one. `.preview-viewport`'s `aspect-ratio: 16 / 10` is that shape, and `object-fit: cover` is only there to guard a shot that is not.
- `data-status` on `.preview-frame` drives the CSS: `loading` (skeleton sweep) → `ready` (the picture fades in), plus `missing` for a template with no shot, whose frame stays a label.
- Cards load `lazy`; the detail page passes `priority`, which is `eager` + `fetchPriority="high"`, because there the picture is the page.
- `.preview-viewport` carries `contain` + `content-visibility: auto` so scrolled-past shots stop costing anything. Its `aspect-ratio` is what holds the box open under `content-visibility` — don't remove it.
