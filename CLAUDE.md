...
The site is a **catalog of work**: light-first, organised into category shelves of cards, each card showing the real site running in a frame. There is deliberately no hero, and no visible page title above the shelves either — the home page opens straight onto the sticky filter bar under the header, with the shelves below it. The `h1` is `.sr-only`, kept for the document outline and for screen readers. Nothing on the site is priced: ordering is arranged over TikTok and the walkthrough lives on `/contact`, not on the home page.
